/**
 * Example C program using tx_tracker FFI
 * 
 * Compile (Windows with MSVC):
 *   cl example_c.c tx_tracker.lib
 * 
 * Compile (Linux/macOS with gcc):
 *   gcc example_c.c -L. -ltx_tracker -Wl,-rpath,.
 * 
 * Run:
...
int main(void) {
    ...
    char* response = tracker_request(handle, create_req);
    if (response) {
        printf("Response: %s\n\n", response);
    ...
    response = tracker_request(handle, list_req);
    if (response) {
        printf("Response: %s\n\n", response);
        tracker_response_free(response);
    }
    ...
    response = tracker_handle_json(activity_req, NULL);
    if (response) {
        printf("Response: %s\n\n", response);
        tracker_response_free(response);
    }
